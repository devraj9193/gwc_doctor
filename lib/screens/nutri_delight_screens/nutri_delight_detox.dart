...
import 'package:doctor_app_new/screens/nutri_delight_screens/recipe_details.dart';
...
import 'daily_progress_screens/symptom_tracker.dart';

class NutriDelightDetox extends StatefulWidget {
  final int userId;
  const NutriDelightDetox({
    Key? key,
    required this.userId,
  }) : super(key: key);

  @override
  State<NutriDelightDetox> createState() => _NutriDelightDetoxState();
}

class _NutriDelightDetoxState extends State<NutriDelightDetox> {
  ...
  getProgramData() async {
    ...
    final result = await ProgramService(repository: repository)
        .getCombinedMealService(widget.userId.toString());
    print("result: $result");
                                  ...
                                  child: GestureDetector(
                                    onTap: () {
                                      Navigator.of(context).push(
                                        MaterialPageRoute(
                                          builder: (ct) => const AllDayTracker(
                                            tabIndex: '0',
                                          ),
                                        ),
                                      );
                          ...
                          GestureDetector(
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => MealPlanRecipeDetails(
                                    mealPlanRecipe: e,
                                    isFromProgram: true,
                                  ),
                                ),
  ...
  getRowHeight() {
    print("MEAL PLAN : ${mealPlanData1.values.length}");
