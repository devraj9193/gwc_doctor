...
class QuickBloxService extends ChangeNotifier {
  ...
  connect(int userId) async {
    print("Connect: $userId");
    try {
      await QB.chat.connect(135990425, AppConfig.QB_DEFAULT_PASSWORD);
  ...
  void sendMessage(
    ...
    try {
      ...
      print("GroupId: $chatRoomId");

      SharedPreferences preferences = await SharedPreferences.getInstance();
      var chatUserId = preferences.getString("chatUserId")!;
      print("UserName: $chatUserId");
      ...
      int senderId = chatUserId as int;
  ...
  Future<List<QBMessageWrapper>> _wrapMessages(
    ...
    for (QBMessage? message in messages) {
      ...
      print(_localUserId);
      wrappedMessages.add(QBMessageWrapper(
          senderName, message, _pref!.getInt(AppConfig.QB_CURRENT_USERID)!));
    }
  ...
  void markMessageRead() async {
    ...
    qbMessage.senderId = (_localUserId == null)
        ? _pref!.getInt(AppConfig.QB_CURRENT_USERID)
        : _localUserId;
